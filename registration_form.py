...
def insert_student_data():
    ...
    if st.button("Insert", key="insert_button"):
        insert_query = "INSERT INTO Students (FName, MName, LName, Title, FatherName, MotherName, ClassID, Gender, DoA, PAddress , EmailAddress, AltEmailAddress, MobileNo, AltMobileNo) " \
                       "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        values = (
            fname, mname, lname, title, father_name, mother_name, class_id.split(" - ")[0], gender, doa, permanent_address,
            email_address, alt_email_address, mobile_no, alt_mobile_no
        )
